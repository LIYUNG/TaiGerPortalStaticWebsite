...
const ExcludeMenu: Record<string, string[]> = {
    Guest: [
        'my-students',
        'user-list',
        'program-table',
...
export const CustomDrawer = ({
    ...
    const userRole = (user?.role as keyof typeof ExcludeMenu) ?? 'Guest';
    const excludeList = ExcludeMenu[userRole] ?? ExcludeMenu.Guest;

    const initialMenuItemOpen =
        MenuSidebar?.filter(
            (menuItem) =>
                !excludeList.includes(menuItem.id) &&
                (menuItem.children?.length ?? 0) > 0
        ).reduce((acc: Record<string, boolean>, menuItem) => {
            acc[menuItem.id] =
                menuItem.children?.some(
                    (subItem) => subItem.url === window.location.pathname
                ) ?? false;
            return acc;
        }, {}) ?? {};
    const [menuItemOpen, setMenuItemOpen] =
        useState<Record<string, boolean>>(initialMenuItemOpen);
    ...
    return (
        ...
        >
            ...
            <List disablePadding>
                {MenuSidebar.filter(
                    (menuItem) => !excludeList.includes(menuItem.id)
                ).map((menuItem) =>
                    menuItem.children ? (
                        <Box key={menuItem.id}>
                            <ListItemButton
