...
const Main = styled('main', {
    ...
    transition: theme.transitions.create('margin', {
        easing: (theme.transitions as { easing: { sharp: string } }).easing
            .sharp,
        duration: (theme.transitions as { duration: { leavingScreen: number } })
    ...
    ...(open && {
        width: `calc(100% - ${drawerWidth}px)`,
        transition: theme.transitions.create('margin', {
            easing: (theme.transitions as { easing: { easeOut: string } })
                .easing.easeOut,
            duration: (
                theme.transitions as { duration: { enteringScreen: number } }
...
const RenderMenu = ({
    ...
    >
        <MenuItem onClick={handleCloseProfile}>
            <Avatar src={user?.pictureUrl as string | undefined} />
...
const NavBar = (props: NavBarProps): JSX.Element => {
    ...
    const [anchorChatListEl, setAnchorChatListEl] =
        useState<HTMLElement | null>(null);
    ...
    const handleNavigateCalendar = (): void => {
        setNavState((prevState) => ({
            ...prevState,
            activeEventCount: 0
        }));
