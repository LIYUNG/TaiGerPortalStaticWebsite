...
interface EmbeddedChatListProps {
    student_id?: string;
}

const EmbeddedChatList = ({ student_id }: EmbeddedChatListProps) => {
    const { user } = useAuth();
    const [searchTerm, setSearchTerm] = useState('');
    const [searchResults, setSearchResults] = useState<unknown[]>([]);
    const { data, isLoading } = useQuery(getMyCommunicationQuery());
    const [chatListState, setEmbeddedChatListState] =
        useState<EmbeddedChatListState>({
            success: false,
            searchMode: false,
            students: [],
            isLoaded: false
